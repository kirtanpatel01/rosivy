...
const Card = ({ card }: { card: ProductCard }) => {
  return (
    <div className="cont1 w-fit flex flex-col items-center gap-2 sm:gap-3">
      <Image src={card.image} alt="green_bukey" height={200} width={200} className="rounded-lg size-36" />
      <div className="w-full flex justify-between">
        <div className="flex flex-col gap-1">
          <span className="font-medium sm:font-semibold text-base sm:text-lg w-32 sm:w-52 text-wrap ">{card.name}</span>
